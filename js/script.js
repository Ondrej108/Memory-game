...
buttonReset.addEventListener("pointerdown", function () {
  location.reload();
});
...
buttonStart.addEventListener("pointerdown", function () {
  ...
  timeDisplay.classList.remove("time-out");
  matchedPairs = 0;
  gameOver = false;
  isLocked = false;
  ...
  shuffled.forEach((img) => {
    img.classList.remove("hidden2", "visible");
...
function shuffleCards(cardList) {
  const shuffled = Array.from(cardList);
